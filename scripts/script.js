window.onload = () => {
  ...
  // msgs
  const msgInputError1 =
    "Please enter only natural numbers (1, 2, 3, 4, etc.) up to 100,000.";
  const msgInputError2 =
    "The numerator cannot be greater than the denominator.";
  const msgRoundedFigures =
    "Note: The probability figures above may not be exact due to rounding.";
  ...
  let displayInput = () => {
    ...
    if (
      inputNumerator === "11" ||
      elevenKRegex.test(inputNumerator) ||
      inputNumerator === "18" ||
      inputNumerator === "8" ||
      inputNumerator.startsWith("8")
    ) {
      aAn.innerHTML = "an";
    } else {
      aAn.innerHTML = "a";
  ...
  let randomStat = () => {
    ...
    outputAns.innerHTML += `<p><span class="text-tertiary-color">That's a probability of ${randomStatProbFixed} (&nbsp;${randomStatPctFixed}% chance&nbsp;), or ${randomStatNumer.toLocaleString(
      "en-CA"
    )}&#8209;to&#8209;${(randomStatDenom - randomStatNumer).toLocaleString(
      "en-CA"
    )} (&nbsp;${randomStatNumer.toLocaleString("en-CA")}&nbsp;:&nbsp;${(
      randomStatDenom - randomStatNumer
    ).toLocaleString("en-CA")}&nbsp;) odds.</span></p>`;
  ...
  let calculate = (event) => {
    ...
    // output to outputAns
    outputAns.innerHTML = `<p>${numerator.toLocaleString(
      "en-CA"
    )} in ${denominator.toLocaleString(
      "en-CA"
    )} (&nbsp;${simpleNumerator.toLocaleString(
      "en-CA"
    )} / ${simpleDenominator.toLocaleString("en-CA")}&nbsp;) is:</p>`;
    outputAns.innerHTML += `<p><span class="text-tertiary-color">a probability of ${decimalFixed} (&nbsp;${percentageFixed}% chance&nbsp;)</span></p>`;
    outputAns.innerHTML += `<p><span class="text-tertiary-color">or ${simpleNumerator.toLocaleString(
      "en-CA"
    )}&#8209;to&#8209;${(simpleDenominator - simpleNumerator).toLocaleString(
      "en-CA"
    )} (&nbsp;${simpleNumerator.toLocaleString("en-CA")}&nbsp;:&nbsp;${(
